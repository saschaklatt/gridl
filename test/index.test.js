...
describe("gridlFactory", () => {
  ...
  describe("validation", () => {
    ...
    it("should throw an error if the data is undefined", () => {
      expect(() => gridl()).to.throw(
        "Trying to import data that is not an array."
      );
    });
    ...
    it("should throw an error if the a rows is not an array", () => {
      ...
      expect(() => gridl([[1, 2, 3], 5])).to.throw(expectedMsg);

      expect(() => gridl([5, [1, 2, 3]])).to.throw(expectedMsg);

      expect(() => gridl([[1, 2, 3], "dsf", [1, 2, 3]])).to.throw(expectedMsg);

      expect(() =>
        gridl([[1, 2, 3], [1, 2, 3], { "0": 0, "1": 1, "2": 2 }])
      ).to.throw(expectedMsg);
    });

    it("should throw an error if the row lengths are not equal", () => {
      const expectedMsg =
        "Trying to import data with inconsistent number of columns.";
      ...
      expect(() => gridl([[1], [1], [1, 2, 3]])).to.throw(expectedMsg);
    });
  });
  ...
  describe("examples", () => {
    ...
    it("should generate a gridl instance", () => {
      ...
      const columns = 4;
      const rows = 3;
      const grid = generators.makeGridl(
        columns,
        rows,
    ...
    it.skip("should draw a cow", () => {
      ...
      const cow = generators
        .makeGridl(13, 6, () => " ") // generate 13x6 grid that is filled with whitespaces
        .setAreaAt([9, 0], head)
        .setAreaAt([3, 2], back)
        .setAreaAt([5, 4], belly)
        .setAreaAt([0, 2], tail)
  ...
  describe("complex operations", () => {
    it("should rotate counterclockwise and flip on x-axis", () => {
      const data = [
        [1, 2, 3],
        [6, 7, 8],
        [10, 11, 12],
